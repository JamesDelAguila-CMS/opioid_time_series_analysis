...
train_data = spark.sql("SELECT * FROM eldb.opioid_SA_LA_hosp_final_tsfresh_and_demos_train_smoteennc")
val_data = spark.sql("SELECT * FROM eldb.opioid_SA_LA_hosp_final_tsfresh_and_demos_ohe where MOD(bene_id, 20) >= 14")

#test_data = spark.sql ("SELECT * FROM eldb.opioid_SA_LA_hosp_final_tsfresh_and_demos_ohe where MOD(bene_id, 20) >= 18")
...
with mlflow.start_run():
  ...
  mlflow.spark.log_model(pipelineModel, "logistic_tsfresh_smote")
