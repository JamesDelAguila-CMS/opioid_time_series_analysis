...
# Function to get Fbeta measures 
...
train_data = spark.sql("SELECT * FROM eldb.opioid_SA_LA_hosp_final_abbr_ftr_extrctn_and_demos_train_smoteennc")
val_data = spark.sql("SELECT * FROM eldb.opioid_SA_LA_hosp_final_abbr_ftr_extrctn_and_demos_ohe where MOD(bene_id, 20) >= 14")
#test_data = spark.sql ("SELECT * FROM eldb.opioid_SA_LA_hosp_final_abbr_ftr_extrctn_and_demos_ohe where MOD(bene_id, 20) >= 18")
...
lr = GeneralizedLinearRegression(labelCol="labels", family='binomial', link='logit', maxIter = 1000, tol=.0000000001) # note GLR uses iteratively reweighted least squares. When positive regParam it is RIDGE (L2)
...
search_space = {
  "regParam": hp.quniform("regParam", .01, 1, .02),
  "tol": hp.quniform("tol", .00000001, .000001, .00000005)
...
with mlflow.start_run():
  ...
  mlflow.spark.log_model(pipelineModel, "logistic_abbr_time_series_smote")
...
display(predDF.groupBy(F.round('prediction', 3)).agg(F.avg('difference')))

# COMMAND ----------

display(predDF.groupBy('labels').agg(F.percentile_approx('prediction', .33), F.percentile_approx('prediction', .66), F.percentile_approx('prediction', .75), F.percentile_approx('prediction', .99)))

# COMMAND ----------

from pyspark.sql import Window
from pyspark.sql.functions import percent_rank,when
w = Window.partitionBy('labels').orderBy('prediction')

percentiles_df = predDF.withColumn('percentile',F.round(percent_rank().over(w), 2))

result = percentiles_df.select(percentiles_df.labels, percentiles_df.percentile, percentiles_df.prediction) #.filter('percentile >= 0.66 and percentile <=1')
                              
display(result.groupBy('labels', 'percentile').agg(F.avg('prediction')))

# COMMAND ----------

display(predDF.groupBy(F.col('labels'), F.round(F.col('prediction'), 1)).count())
