...
train_data = balanceDataset(spark.sql("SELECT * FROM eldb.opioid_SA_LA_hosp_final_tsfresh_and_demos_ohe where MOD(bene_id, 20) < 14").drop("CC_Depressive_dis"))
val_data = balanceDataset(spark.sql("SELECT * FROM eldb.opioid_SA_LA_hosp_final_tsfresh_and_demos_ohe where MOD(bene_id, 20) >= 14").drop("CC_Depressive_dis"))
#test_data = balanceDataset(spark.sql ("SELECT * FROM eldb.opioid_SA_LA_hosp_final_tsfresh_and_demos_ohe where MOD(bene_id, 20) >= 18"))
...
#   best_tol = best_hyperparam["tol"]
  ...
  cv = CrossValidator(estimator=estimator, evaluator=binaryClassificationEvaluator, estimatorParamMaps=paramGrid, numFolds=5, seed = 7809051150349531440) 
...
# MAGIC %md
# MAGIC Get past model

# COMMAND ----------

# import mlflow
# logged_model = 'runs:/6af0b16925024a9d92e405b12b77a8ae/logistic_tsfresh_classweights'
